...
- 格式感知解析：通过可扩展 Extractor 提取 PMX、VMD、FBX、Blend 和 UProject 的可靠结构化元数据，并明确标记解析来源、状态和错误。
...
- 语义检索：只对人工描述、正式标签、作者信息或格式内嵌语义生成 `BAAI/bge-m3` 1024 维向量；路径、扩展名和不透明二进制文件不会单独获得向量。
